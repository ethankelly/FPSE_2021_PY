import random
from src.adapter_soln import PlayingCard
from src.adapter_soln import ConsoleInput
from src.adapter_soln import ConsoleOutput
...
class BlackJack:
    ...
    def setGameInput(self,game_input):
    ...
    def find_winner(self, hands):
     ...
     the to the previous player list. To determine a winner if you have the same score you win if you have five cards."""
        ...
        for counter in range(0, len(hands)):
            ...
            if previous_score < current_score or (previous_score == current_score and (len(hands[counter])==self.good_number_of_cards and len(previous_hand) != self.good_number_of_cards)):
