import random
from adapter_demo.PlayingCard import PlayingCard
from adapter_demo.ConsoleInput import ConsoleInput
from adapter_demo.ConsoleOutput import ConsoleOutput
...
class BlackJack:
    ...
    def valid_deal_input(self):
        ...
        Gets an input of "D"eal or "S"tick from the user, validates only "D" or "S" has been entered and the
    ...
    def deal_to_user(self, deck, hand):
        ...
        can stick so stop and move on. When you are dealt a card we determine the score, if you go over the limit 21 you
        lose and are bust. In this case we move on.
        """
        answer = "D"
        while answer == "D":
    ...
    def find_winner(self, hands):
        ...
        replace the previous score, so we only store the highest score so far. If we have a draw both players are added
        to the to the previous player list. To determine a winner if you have the same score you win if you have five
        cards.
        """
        previous_player = []
        previous_hand = []
    ...
    def deal_to_computer(self, deck, hands, computer_risk):
        ...
        if there risk level is nine they will stick if the have a score of twelve or above. If they have nine they will
        request an additional card. One gap is the computer does not know if they have an Ace. If the have an ace it
        could be sensible to request another card, this would require a second risk level for when you have an Ace.
        """
    ...
    def black_jack(self, deck, hands, computer_risk):
        ...
        of risk for the computer. The cards need to add up to 21 or less. If you go above 21 you are bust (loose). First
        the user has the opportunity to request more cards to be dealt, one at a time. The computer will have a risk,
        the number on or above they will stick at i.e. not ask for more cards. Once all the cards have been dealt we
        determine the winner, i.e. the closest to 21.
        """
